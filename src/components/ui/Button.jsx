...
const Button = forwardRef(({
  ...
  const variants = {
    ...
    'ghost-danger': {
      base: 'bg-transparent text-red-300 border border-red-400/30 backdrop-blur-sm',
      hover: 'hover:bg-red-500/10 hover:border-red-400/60 hover:text-red-200 hover:shadow-red-500/20',
      disabled: 'bg-transparent text-gray-400 border-gray-600 cursor-not-allowed'
    },
    danger: {
  ...
  const handleClick = useCallback((event) => {
    ...
    try {
      ...
      logger.error('Button click handler failed:', error);
    }
  }, [disabled, loading, onClick, soundEnabled]);

  const handleMouseEnter = useCallback(() => {
    if (disabled || loading || !soundEnabled) return;

    try {
      playHover();
    } catch (error) {
      logger.warn('Button hover sound failed:', error);
    }
  ...
  return (
    ...
    >
      ...
      {!loading && (
        <>
          {icon && size !== 'icon' && iconPosition === 'left' && (
          ...
          {icon && size !== 'icon' && iconPosition === 'right' && (
            <span className={iconSpacing}>
              {icon}
            </span>
          )}
